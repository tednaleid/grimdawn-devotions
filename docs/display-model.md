...
3. **Emphasis** (a union of additive effects) <- independent cues that genuinely stack. Active elements glow, selected elements get special styling, taken edges glow gold, search-matched stars wear split rings colored per search, a text search halos its matching constellations, compare-diff shows outlines. These are not competing for a single property; they are additive layers that coexist.
...
- **Search rings**: a star matched by one or more active searches wears a ring outside its dot. Every selected benefit tag is its own search, and the text query is one more, so the ring carries identity: one matching search draws a full ring in that search's style; several split the ring into equal arcs, clockwise from twelve, in ring order. A style is a color plus a stroke dash pattern - the pattern is the redundant channel that keeps searches apart under red-green color vision deficiency and at heavy zoom-out. Each arc's stroke width also scales with the star's relative magnitude for that search (the smallest grant among the search's matches renders at base width, the largest at quadruple; text-query matches have no magnitude and stay at base), so a +80% star visibly outweighs a +30% one and each slice of a split ring scales independently. Weighted arcs grow outward from a fixed inner edge, keeping the star dot visible, and their dash patterns scale with the width so dots stay dots at any size. The color and dash cycles are deliberately different lengths (five colors including white, four patterns), so the combined style stays unique for twenty concurrent searches instead of repeating with the palette. A star's arc order is canonical (URL-bitset order), but style slots are session-stable: removing a tag frees only its own style and every other tag keeps its own, and a new tag takes the least-used free slot - stability while editing beats shared-link color fidelity, so a fresh page load reseeds slots in canonical order and may wear different styles while marking the same searches. The query always keeps its reserved style, and the same styles key the chrome: selected Benefits rows, the search box, and tagged bonus rows in the tooltip are outlined in their color and carry a mini patterned ring swatch (the sidebar is the legend). Ordering, slot reconciliation, and the per-star fold live in `core/searchRings.ts` (slots only, no colors, per the core discipline below); the style palette and swatch live in `adapters/ringPalette.ts`. Rendered as its own full-opacity layer so the ring reads even on an unattainable (dim) star. When the star's constellation fails the affinity filter, the ring is wrapped in a desaturate filter so the whole effect reads as "search match, off-filter" without the star's opacity bleeding through.
- **Search-match halo**: a constellation whose name or description matches the active text search glows via a `#search-glow` halo, built the same way as the affinity halo (`#aff-glow`: wide blur, stacked merge for alpha density) but flooded with the query's reserved ring color, so a query hit reads the same whether it lands on a star (a ring) or a whole constellation (this halo) and never reads as an affinity color. Like the affinity halo, its opacity respects the brightness channel (dimmer on an unattainable constellation) and it desaturates via `#mute-wide` when the constellation is muted by an affinity filter, so a search hit stays visible on an off-filter constellation instead of vanishing.
...
Crucially, the affinity filter only *emphasizes* matches. It does not de-emphasize non-matches. A search-matched star in an off-filter constellation wears its ring (search cue) and desaturates (affinity cue) *at the same time*, so you can still see it matches your target even though it is off-filter.
