...
3. **Emphasis** (a union of additive effects) <- independent cues that genuinely stack. Active elements glow, selected elements get special styling, taken edges glow gold, search-matched stars wear clock hands (one per search), a text search halos its matching constellations, compare-diff shows outlines. These are not competing for a single property; they are additive layers that coexist.
...
- **Search hands**: a star matched by one or more active searches wears one clock hand per search: a straight, round-capped line about as wide as the dot, radiating from the star's centre over the dot at that search's fixed angle, in its color, with a faint track circle around the dot. Every selected benefit tag is its own search and the text query is one more, so identity is angle first and color second: a search points the same way on every star it marks, which survives red-green color vision deficiency and heavy zoom-out (the hands cover the dot's hub; the dot's rim still shows its affinity color). A style is a pure function of the search's slot: eight angles (cardinals first, then diagonals, straight up last) cycle against five colors (white, cyan, orange, pink, lavender: white because lightness survives every color deficiency, the rest sitting between the affinity hues), so the combined style stays unique for forty concurrent searches and slots five apart (the same color) always point at least 135 degrees apart. The query keeps a reserved style, straight up in lime. Hand length carries the star's relative magnitude for that search (the smallest grant among the search's matches renders at base length, the largest almost four times as long; text-query matches have no magnitude and stay at base), so a +80% star visibly outreaches a +30% one, read as length from the centre rather than as width. Searches that share an angle (slots eight apart, or the query with the eighth benefit slot) stack end to end along the ray, the query or lower slot at the root, and a stack's tip is clamped one minimum segment past a full-length hand. Style slots are session-stable: removing a tag frees only its own slot and every other tag keeps its own, and a new tag takes the least-used free slot; stability while editing beats shared-link fidelity, so a fresh page load reseeds slots in canonical order and may wear different hands while marking the same searches (the "3 o'clock pink" a sender sees is not promised to the recipient). The same styles key the chrome: selected Benefits rows, the search box, and tagged bonus rows in the tooltip are outlined in their color and carry a mini-star swatch wearing the same hand (the sidebar is the legend), and hovering one of them pulses that search's hands on the map for about five seconds (an opacity dip, none under reduced-motion), so the legend can point at its hands. Ordering, slot reconciliation, weights, and the per-star fold live in `core/searchMarks.ts` (slots only, no angles or colors, per the core discipline below); the angle and color tables and the swatch live in `adapters/handPalette.ts`; the geometry (root, lengths, stacking) is the renderer's, with the constant hand width, dark outline, and track look in CSS. Rendered as its own full-opacity layer so the hands read even on an unattainable (dim) star. When the star's constellation fails the affinity filter, the hands are wrapped in a desaturate filter so the whole effect reads as "search match, off-filter" without the star's opacity bleeding through.
- **Search-match halo**: a constellation whose name or description matches the active text search glows via a `#search-glow` halo, built the same way as the affinity halo (`#aff-glow`: wide blur, stacked merge for alpha density) but flooded with the query's reserved hand color, so a query hit reads the same whether it lands on a star (a hand) or a whole constellation (this halo) and never reads as an affinity color. Like the affinity halo, its opacity respects the brightness channel (dimmer on an unattainable constellation) and it desaturates via `#mute-wide` when the constellation is muted by an affinity filter, so a search hit stays visible on an off-filter constellation instead of vanishing.
...
Crucially, the affinity filter only *emphasizes* matches. It does not de-emphasize non-matches. A search-matched star in an off-filter constellation wears its hands (search cue) and desaturates (affinity cue) *at the same time*, so you can still see it matches your target even though it is off-filter.
