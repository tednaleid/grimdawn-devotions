...
- just perf (median / p95 / p99) and the chosen live budget: resolver WASM (data/reach.wasm); per-click latency over 440 clicks: mean 4.3 ms, median 2.5 ms, p95 15.0 ms, p99 23.6 ms, max 29.0 ms; 0 clicks over 400 ms. Envelope rule: keep the live default `tries = 16` if p95 <= 45 ms AND p99 <= 190 ms. Both hold (15.0 ms and 23.6 ms), so the live default stays `tries = 16`; no code change. `buildOrderEscalated` keeps `tries = 4096` (per-build cost 2.0 s, the harness's worst case), unchanged and off the per-click path.
