...
- Synthetic corpus aggregate and new pins (Task 9): `aggregate: orders=150/150 churn=19 steps=2591 | repro: churn=4 steps=23`, down from the baseline churn=22 steps=2701. Pins moved to `CHURN_PIN = 20` (was 36) and `STEPS_PIN = 2643` (was 2766), each the measured value plus ~2% slack; `ORDER_FLOOR` stays 150 and the repro build's pins stay 4 / 23.
- Real corpus @ tries=16 / 256 / 4096 (the true headroom curve, Task 9):
  - `real corpus @ tries=16: orders=99/99 churn=339 steps=1955 divergent=15 mean_ms=8.9`
  - `real corpus @ tries=256: orders=99/99 churn=302 steps=1895 divergent=16 mean_ms=127.2`
  - `real corpus @ tries=4096: orders=99/99 churn=255 steps=1807 divergent=13 mean_ms=2010.1`
  - The baseline's flat 382 / 2019 became a real curve: 339 at the live budget, 255 at the escalation budget, a 33% churn cut with steps falling alongside rather than trading against it. Cost is now linear in `tries` because quality mode never short-circuits.
- just perf (median / p95 / p99) and the chosen live budget: resolver WASM (data/reach.wasm); per-click latency over 440 clicks: mean 4.3 ms, median 2.5 ms, p95 15.0 ms, p99 23.6 ms, max 29.0 ms; 0 clicks over 400 ms. Envelope rule: keep the live default `tries = 16` if p95 <= 45 ms AND p99 <= 190 ms. Both hold (15.0 ms and 23.6 ms), so the live default stays `tries = 16`; no code change. `buildOrderEscalated` keeps `tries = 4096` (per-build cost 2.0 s, the harness's worst case), unchanged and off the per-click path. Correction to the plan's Task 10 wording: there is no "Find valid order" button - `buildOrderEscalated` has no caller outside `web/test/`, so the tries=4096 churn (255) is available to the harness and to a future control, not to users.
- 32-tries trial (the doubling Task 10 predicted would pay, run afterwards): real-corpus churn 334 at tries=32 against 339 at tries=16 (steps 1943 against 1955, harness mean 15.5 ms against 8.4 ms per build); `just perf` at tries=32: mean 5.3 ms, median 3.0 ms, p95 19.0 ms, p99 29.1 ms, max 39.6 ms, 0 clicks over 400 ms. Decision rule was adopt only if p95 <= 45 ms and churn is at least 5 points below 339; both hold (19.0 ms, exactly 5 points), so ADOPTED: `buildOrderPath`'s default `tries` and the `selectionView` call site are 32. The gain is marginal (1.5 percent of churn for roughly double the per-click search) and it clears the rule at the boundary. The churn/steps pins and the harness ladder still pass an explicit 16, so `just order-quality`'s 16 rung is the ladder's floor rather than the live setting; `web/test/real-build-order.test.ts`, whose subject is the live path, moved to 32 with it.
- Testing deviation from the spec: the spec asked for a unit test on a constructed build where continuing past the first fit provably lowers churn, but `sampledConstruction` and `emitSchedule` are private to `reachability.ts`, so the mode is not addressable from a test; the pins are corpus level instead (`web/test/order-quality-mode.test.ts`: strict aggregate improvement of a larger budget over tries=16, plus the witness-order snapshot over seeds 1..20 in `web/test/build-order.test.ts`, which pins that witness mode still stops at the first fit).
- Divergence counter across both corpora: 15 / 16 / 13 of 99 real builds, against a baseline of 5. The counter's definition widened in Task 9: the steps-first alternative it compares against now includes `samplerStepsFirst`, a dedicated steps minimizer the baseline did not have, so the baseline 5 and these values are not a like-for-like change and the rise is not by itself evidence that the objective matters more than it did. The churn and steps totals above ARE like-for-like: `pick` is computed over the two shipped generators by `buildOrderPath`'s own rule, and it was measured equal to `buildOrderPath`'s output with zero builds differing.
