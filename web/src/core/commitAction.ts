...
export function commitButton(
  ...
  if (target.kind === "star") {
    if (selected.has(target.id)) return { label: translate("ui.commit.remove"), enabled: true };
    return { label: translate("ui.commit.add"), enabled: reach.clickable.has(target.id) };
  }
  ...
  if (starIds.length > 0 && starIds.every((id) => selected.has(id)))
    return { label: translate("ui.commit.remove"), enabled: true };
  return { label: translate("ui.commit.add"), enabled: reach.completable.has(target.id) };
}
