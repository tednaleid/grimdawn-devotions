...
test("a tagged bonus row wears its search's mark style: color outline and mini-star swatch", () => {
  const star = [...model.stars.values()].find((s) => Object.keys(s.bonuses).length > 0)!;
  const probe = el();
  ...
  tip.setMarkStyles(new Map([[vid, { angle: 90, color: "#3ee6d8" }]]));
  const e2 = el();
  ...
  expect(html).toContain(`class="tip-bonus vsel" data-vid="${vid}" style="--mark:#3ee6d8"`);
  expect(html).toContain('<svg class="mark-swatch"');
  expect(html).toContain('<path d="M 11.96 4.04 A 5.6 5.6 0 0 1 11.96 11.96" fill="none" stroke="#3ee6d8"');

  // Mark styles are module state (like the query highlight); clear them for other tests.
  tip.setMarkStyles(new Map());
  const e3 = el();
  tooltipView(e3).show(enLoc, model, star.id, 0, 0, undefined, undefined, new Set([vid]));
  expect((e3 as any).innerHTML).not.toContain("mark-swatch");
});

test("a tagged celestial-power stat row wears vsel and its mark style, like bonus rows", () => {
  ...
  const e = el();
  const tip = tooltipView(e);
  tip.setMarkStyles(new Map([[vid, { angle: 180, color: "#ff9440" }]]));
  tip.show(enLoc, model, star.id, 0, 0, undefined, undefined, new Set([vid]));
  const html = (e as any).innerHTML as string;
  expect(html).toContain(`class="tip-bonus vsel" data-vid="${vid}" style="--mark:#ff9440"`);
  expect(html).toContain('<svg class="mark-swatch"');
  tip.setMarkStyles(new Map());
});
